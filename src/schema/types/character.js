...
const { getData, getDataById, getDataListById, addGender } = require('../apiHelper')
...
const charType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    father: {
      ...
      resolve: (parentVal, args) => getDataById(charactersJSON, parentVal.Father)
    },
    mother: {
      type: charType,
      description: 'Characters Father',
      resolve: (parentVal, args) => getDataById(charactersJSON, parentVal.Mother)
    },
    spouse: {
      type: charType,
      description: 'Characters spouse',
      resolve: (parentVal, args) => getDataById(charactersJSON, parentVal.Spouse)
    },
    ...
    povBooks: {
      type: new GraphQLList(bookType),
      resolve: (parentVal, args) => getDataListById(booksJSON, parentVal.PovBooks)
    },

    children: {
      type: new GraphQLList(charType),
      resolve: (parentVal, args) => getDataListById(charactersJSON, parentVal.Children)
    },

    allegiances: {
      type: new GraphQLList(houseType),
      resolve: (parentVal, args) => getDataListById(houseJSON, parentVal.Allegiances)
    }
  })
})
